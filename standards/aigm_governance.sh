...
AIGM_GOVERNANCE_VERSION="3.0.0-alpha.1"
...
AIGM_GOVERNANCE_RULE_origin_protects_aigm_integrity_not_domain_content="Origin identifies and recognizes Tier-1337-capable actors and safeguards AIGM meta-standard integrity. Origin MUST NOT control domain-standard content unless that content violates an applicable AIGM invariant."
...
AIGM_GOVERNANCE_RULE_promotion_requires_general_three_validation_rule="Except for an explicitly non-promotable Origin role, promotion to a target tier requires three independently validated approvals from actors already holding that target tier together with satisfaction of the target-tier requirements."
